...
describe('dynamicStep — proportional wheel nudge (~4%), floored at base', () => {
  it('scales with the value (chip load) — halves as the value halves', () => {
    expect(dynamicStep(0.010, 0.0001)).toBeCloseTo(0.0005, 6);
    expect(dynamicStep(0.005, 0.0001)).toBeCloseTo(0.0002, 6);
  });

  it('never drops below the field base step (still nudgeable near/at zero)', () => {
    expect(dynamicStep(0.003, 0.0001)).toBeCloseTo(0.0001, 6);
  ...
  it('is proportional for large fields too (RPM, feed)', () => {
    expect(dynamicStep(8000, 10)).toBe(200);
