...
**Flagged, never auto-corrected.** `stockMaterialIssues(presets, materials)` returns each offending preset with what Fusion holds (`unknown[]`) and what its own CAM-preset FK implies (`expected`); **`MaterialLinkBanner`** shows them on the tool page. Which material such a preset should now cut is a real editorial decision (the old assignment encoded someone's intent), so the app states the problem and the user re-picks in the preset editor. Automating a fix is deliberately deferred — the flag is the feature. ⚠️ **Both writers must PRESERVE a non-matching `stock-materials`, or the flag can never fire**: `syncPresetMaterialName` and `FUSION_PRESET_PATCHERS.material` apply the identical rule (rewrite only when it's absent, equals the new name, or equals the OLD query — i.e. plainly derived from the name being corrected). They are the two places that touch this one field and they must not drift; clobbering it in either would destroy the only evidence the reference was ever stale. Locked by `presetNaming.test.js` + `presetMaterialPush.test.js`.
