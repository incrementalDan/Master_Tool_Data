...
**Material comes from the Materials library** (`materials.json` — see Shared Drive Files): it is the **single source of material in the app**, a **3-tier taxonomy** — ISO **groups** (P/M/K/N/S/H) → **CAM presets** (the speed/feed preset name pushed to Fusion, carrying each standard's code: ISO 513 / Kennametal / Haas-VDI 3323) → **materials** (individual alloys, each with `aliases[]` for "look it up by the name we know it by" and a `preset_id` linking up to a CAM preset). The preset material picker in `PresetPanel` is two dropdowns — **Group** (P/M/K/N/S/H + custom) → optional **CAM Preset** (the presets in that group) — sourced from `state.materials`. The selection is stored in the preset's Fusion-native `material.query` as the **CAM preset name** (if picked) else the **group label** (e.g. `"SS Austenitic 316"` / `"Stainless Steel"`); Fusion accepts any string. The old hardcoded `MATERIALS` / `MATERIAL_QUERY_MAP` list in `PresetPanel` was removed.
...
- **`MaterialsEditor.jsx`** (`/materials`) — the 3-tier editor + a top **search/lookup** (matches alloys, aliases, CAM presets, and codes) and an ISO group-filter that drives the lower two sections. Three sections: **Material Groups** (drag-reorder via `useDragReorder`, editable color/label/**code**, ISO groups not deletable, `+ Add Group`), **CAM Presets** (expand-to-edit row: name/code/description/group + the three standard codes, `+ Add CAM Preset`), and **Materials** (expand-to-edit alloy: label/aliases/group/linked CAM preset/condition/code/codes/notes, `+ Add Material`). A **"Load reference data"** button resets the whole library to the bundled seed (one-off migration), and **"Copy JSON"** exports the full taxonomy. Autosaves to `materials.json` on each change via `saveMaterials`. **This library is the only source of material** in the app (the preset picker + naming + coloring all read it) and **group colors drive preset color coding** — see Preset color coding below.
