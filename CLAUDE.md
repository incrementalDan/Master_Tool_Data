...
- **Tolerances and epsilons** — `lengthEps`, `PRESET_SIGNIFICANCE`'s `len: true` floors, `readAboveGaugeFlags`. An inch-sized tolerance is 25× too loose in mm. ⚠️ But scaling a tolerance is NOT automatically the metric fix, and reaching for it can hide the real bug: `snapTol(unit)` scaled the naming tolerance up for a metric tool while still comparing a **millimetre** value against **inch** reference charts, so it only widened the window on a comparison that should never have run — see the drill/fraction rule below.
...
**Rule** (`internalToFusionTool`): for **existing** presets, never ADD an expression key; for keys that are present, keep the original string **byte-for-byte when the paired numeric is unchanged** (preserves formulas and native formats — `approxEqual` absorbs Fusion's float noise), and rewrite when the value changed (compared against `existingPresetByGuid`) **or when a LITERAL already disagrees with its numeric**. "Both absent" (no numeric, no change) also preserves — some native presets carry only the expression (e.g. drill `tool_feedPerRevolution`) and Fusion derives the numeric from it. Fusion's default formula set (`tool_spindleSpeed` ternary, `tool_feedPlunge` ternary, ramp/transition/retract links) is seeded **only for blank app-created presets** (`isBlankPreset`). **One speed mode + one feed mode only**: Fusion stores exactly ONE of `tool_spindleSpeed`/`tool_surfaceSpeed` and ONE of `tool_feedCutting`/`tool_feedPerTooth`/`tool_feedPerRevolution` per preset — never both (verified: 0 co-occurrences across 345 real reference presets). So blank seeding writes `tool_spindleSpeed` and `tool_feedCutting` (RPM + cutting-feed mode) but **NOT** the `tool_surfaceSpeed` or `tool_feedPerTooth` companion *expressions* — seeding those makes Fusion flag the tool on load and strip them ("warning, then fixes itself when opened"). The paired **numerics** `v_c`/`f_z` are still seeded (Fusion stores them standalone); only the redundant expressions are omitted.

⚠️ **"DID THIS WRITE CHANGE THE VALUE" IS NOT ENOUGH ON ITS OWN — a literal that ALREADY disagrees with its numeric is stale, and the old rule could not see it.** Fusion re-derives the numeric from the expression on load, so `f_n: 0.0056` sitting next to `".002 in"` runs at **.002**. The failure is that the bad state is **self-perpetuating**: every later save compares the value against the stored entry, finds it unchanged, and preserves the stale string — so the app could never heal it, no matter how many times the tool was saved. Nothing on screen showed it either; the app's tool page and Fusion's own preset table both read the *numeric*, so both looked right while the machine would have run something else. The heal is therefore keyed on **the expression disagreeing with the number**, not on this write having moved it (`literalExprNum`, `fusionConvert.js`). Found on drill **D-43**, whose Fusion entry carried `f_n 0.0056` next to `".002 in"` and, on a copied preset, `v_c 200` next to `"250 fpm"`.

⚠️ **A FORMULA is still only rewritten when the value CHANGED — never on disagreement.** Its value cannot be evaluated here, so "disagrees" is unanswerable; and the blank-preset seed deliberately writes formulas (the `tool_feedPlunge` ternary, `tool_feedRetract`/`tool_feedRamp` links) that must survive untouched until someone actually edits the field. `literalExprNum` is strict for exactly this reason: a number followed by nothing but unit letters and the `/` of `"m/min"` — so every formula, which starts with an identifier or a paren, is rejected. ⚠️ The `/` is not cosmetic: `speedUnit` is `fpm` in inches but **`m/min`** in mm, so a letters-only test would have left every metric surface speed unhealable.

⚠️ **A COPIED preset must DROP the source's `expressions`** (`copyOfPreset`, `PresetPanel.jsx`). It is Fusion's derivation of the *source's* numbers, so an inherited literal asserts the original's value for any field the copy doesn't happen to change, and an inherited formula silently governs a copy nobody set one on. A copy is a new preset with a new guid and its own numerics — like `blankPreset()` it carries none, and Fusion derives them from the numbers. Exported purely so it can be test-locked (`presetCopy.test.js`); both of the things it drops are invisible in a rendered snapshot.

**Measured on the real library** (`ToolDEX - MASTER 8-10-26.json`, 303 tools / 671 presets): **17 stale literals across 3 tools** — including `A-23`, whose Fusion entry said 14000 rpm against the app's 16000 and 20 in/min plunge against 50. They heal on each tool's **next ordinary save**; nothing bulk-rewrites them. The round-trip audit stays at **0 unexpected diffs** across all 233 reference tools, so the heal only ever fires on a genuine disagreement. Locked by `fusionConvert.test.js` (`stale literal preset expressions self-heal`), incl. idempotence, the float-noise case, the expression-only native preset, both formula cases, and the metric `m/min` case.
...
⚠️ **EDITING AN OOH ONTO A VALUE ANOTHER ASSEMBLY ALREADY HAS CREATES A DUPLICATE, and nothing used to say so.** Instances of one logical tool differ **ONLY** by holder and OOH — that is the whole multi-instance model — so two assemblies agreeing on both are one setup recorded twice, and the tool sits in **Fusion two or three times identically**. `classifyStrays` (`reconcile.js`) does classify a duplicate, but only among **STRAYS**: these are properly *registered* (own `instance_guid`, live in Fusion, holder FK set), so reconcile never looks at them and the state was unreportable and unfixable. Same shape of gap as the rest of `libraryHealth.js` — the app declines to *create* it while having no way to *report* one already there. Detector + healer: **`duplicateAssemblyIssues` / `dedupeAssemblies`** (`src/utils/libraryHealth.js` §4), driven by the **Assemblies recorded twice** block of Settings → Library Health (`findDuplicateAssemblies` / `fixDuplicateAssemblies`, preview→commit, per-tool selection).
- ⚠️ **THE HOLDER IS COMPARED BY RESOLVED RECORD, NEVER BY RAW GUID.** Fusion re-issues holder guids, so one physical holder legitimately appears under two guids on one tool — the real `A-253`, where one assembly carries a live `holder_id` and the other a guid resolving to nothing. Grouping on the guid misses it: the **Relational integrity** rule applied to a *comparison*. Both resolve → the **record** decides (two distinct records are two distinct holders, even with matching descriptions — merging there would paper over a holder-library duplicate instead of reporting one); otherwise the **description** decides, because a holder's name IS its description and it is the only signal an unresolvable assembly has left.
- ⚠️ **TWO FIELDS VETO A MERGE rather than being quietly dropped**: a differing `measured_gauge_length` (the IMMUTABLE presetter reading — two readings on one stickout is a real disagreement only the shop can settle) and differing `notes` (prose the app cannot merge without inventing an order). Both are still **reported**, just not merged. The survivor is **the assembly carrying a measurement**, else the first; `linked_preset_guids` and `legacy_asm_numbers` are **unioned** (a preset pointing at a dropped assembly would be orphaned; a retired number is searchable history).
- The dropped assemblies need **no Fusion delete of their own** — `writeLogicalTool` rebuilds a tool's entry set from its assemblies, so the redundant instances simply stop being written. That is also why the fix is **TOOL-scoped** (`writeToolsToFusion`), not a field push.
- ⚠️ Stickout is matched with the shared **unit-aware** `lengthEps`, so float noise never splits a pair and a real 0.001" difference never joins one; `dedupeAssemblies` returns the **same tool reference** when nothing merges (the identity contract every healer here follows).
- **Measured on the real library: 10 groups across 10 tools, 11 redundant assemblies, 0 preset links lost, second run a no-op.** Two of them (`A-139`, `A-166`) still carry the stale Auto `asm_number`s from before the edit that collapsed them — `…-A-139-1.7` and `…-A-139-1.85` on two assemblies whose stickout is now 1.88 in metadata **and** in Fusion's own `geometry.LB`. Locked by `libraryHealth.test.js`.

⚠️ **A number spelling the holder the RETIRED short way is stale, and is corrected.** `backfillAsmNumbers` used to run its compare through `holderTokensMatch`, so `30-SK13-60-1001-2.125` was judged EQUAL to the freshly-composed `NBT30-SK13C-60-1001-2.125` and never recomputed — no save, re-stamp, or bulk write could ever fix it. The compare is **strict**, so the number self-heals on the tool's next write (an ordinary save, or `writeToolsToFusion` — the holder re-stamp pass, which persists metadata too). Locked by `assemblyIdSystem.test.js`.

**Load-time correction of a stale Auto `asm_number`** (`backfillAsmNumbers`). The edit path above can't catch every source of staleness — the OOH can change in Fusion, a Tool ID renumber changes the id token, a holder description can change, and assemblies may pre-date Auto mode being configured. Because an Auto number is a **pure product** of holder + tool_id + OOH **and has no edit UI** (`AssemblyForm` exposes the field only in `proshop_rta` mode), a stored value that differs from the composed one is always **stale, never custom** — so `backfillAsmNumbers` **re-derives** it (it previously only filled a missing one).

⚠️ **THE CORRECTION IS SILENT — do not re-add a banner for it.** There was one: a runtime `_asmNumbersFixed` array (`[{from, to}]`) driving a blue "N assembly numbers … corrected — Save" notice in `ToolDetail`. It was **removed**, for two reasons that both still hold. **It had nothing to decide** — the only action it offered was a save the app performs on the tool's next write anyway, over a value that is a *reference*, not a link (the real links are `preset_meta.assembly_id` and `assembly.holder_id`), so a stale one costs nothing while it waits. And **it read as un-clearable**: the flag is set at load and the saved tool carried it forward in memory, so pressing Save stored the corrected numbers and left the banner sitting there until a page reload — the "flag that re-fires after it's been dealt with" failure in checklist question 8, in its quietest form. What the user actually watches is the assembly description, which is derived and already correct.

⚠️ **The checker MUST compose exactly like the stamper** (`writeLogicalTool`) — same holder resolution (cached `holder_description`, then the holder library by `holder_guid`) and the same `trimOoh` rounding. If the two ever disagree, every load rewrites what the last save stored and the record never settles (with no banner, that now shows up as a tool that is permanently dirty rather than as a false alarm). Both causes are regression-tested. This is **strictly `auto`-mode**: the function early-returns for every other mode, so `proshop_rta` / `erp_external` / `sequential` numbers — which are NOT derived from these fields — are never touched. In-memory at load, persisted on the next save; idempotent, so a correct library is a no-op and `backfillAsmNumbers` returns the **same array reference**. ⚠️ Corollary: switching an existing shop **from `proshop_rta` to `auto`** means those RTA numbers get re-derived at load — the Auto value is the source of truth in that mode.
...
- ⚠️ **Only a GET is ever retried (`shouldRetryRequest`).** Every POST in `apsService.js` CREATES something — a storage location, or a new version of the shop's library — so if the request succeeded and only its response was lost, a retry makes a **second one**. A rate limit that surfaces as an error is a nuisance; a duplicate library version is real damage. Retries cover `429` and `5xx` only, with capped exponential backoff honouring `Retry-After`, which is what makes the bulk read passes (`renumberLibrary`, `assignToolIds`, `renumberAllToolIds` — they download *every* linked library) survivable. **Never extend retries to a write.**
- ⚠️ **Hub/project/folder listings are PAGED — read them with `apiFetchAll`, never `data.data`.** `page[limit]` defaults to 200 and cannot exceed it, so a single read of a longer collection returns a **truncated list with no error**: in the library picker the file simply isn't listed, and it reads as "the app can't see my file". `apiFetchAll` follows `links.next` (tolerating both the object and string shapes), stops on a self-referential link, and is a no-op single pass when no next link is sent.
- ⚠️ **A library save REFUSES to overwrite a version it did not read (`versionMovedSince`).** A write **replaces the whole file for the whole shop**, so the loser of a save race loses everything they changed — with a success message on both screens. `loadToolLibraryWithVersion` records which version was read; `saveToolLibrary` re-checks before creating anything and throws `LIBRARY_VERSION_CONFLICT` if it moved, having written nothing. This matters most for `saveFullLibrary`, which writes the complete in-memory set and **never re-downloads**. ⚠️ **Only a clear, positive mismatch blocks** — an unrecorded version, a failed check, or an uncomparable pair all let the save through, because a save that wrongly refuses is its own kind of loss (the edits exist only on the user's screen). Never compare version numbers with plain `Number()`: `Number(null)` is `0` and would block a valid save.
- ⚠️ **`versions:autodesk.core:File` in `saveToolLibrary` is the ONLY line in the app that depends on hub style — check it before migrating the tool library.** Nothing else reads hub or project type; every other call just walks hub → project → folder → item, which is identical across hub types. `autodesk.core` is the Fusion Team / A360 lineage; BIM 360 and Forma projects use their own variant, and the Data Management docs identify those by a **`b.` prefix on the project id** (ours carries `a.`, and saves work — so it is right today, though the Create Version reference does not enumerate the valid values, making that evidence rather than proof). The shop's hub has been converted to the new **collaborative** style but the tool library has deliberately NOT been migrated. **When it is, this line is the first thing to verify** — the failure mode is a failed save on day one, not silent corruption.
...
⚠️ **MIN OOH IS THE HOLDER FACE, NOT A LENGTH OF THE TOOL** — it is where the holder starts, usually the face of the collet nut. Nested in the left-hand dimension stack it read as one more length of the cutter, sorted in among the flutes and the shoulder. It draws instead as a **dotted datum across the drawing**, wider than the tool so it reads as a plane the tool passes through, in **burnt orange** (`--tp-holder-line`, its own token beside the drawing's other colours) — in the dimension grey it vanished over the body and its overhang was indistinguishable from a diameter leader crossing at the same height. Deliberately redder than the golden flutes (`--tp-flute`) so a thin dashed line never reads as part of the cutting portion, lifted a little because a dash needs more luminance to carry on the dark ground than a filled region does, and drawn **thicker than a dimension line** — a datum is a plane, not a measurement. Its **value box sits in the ordinate column with the tool's own lengths** — it is measured from the same assumed datum, so a print puts it in the same run of values, and its colour is what keeps it reading as a different kind of thing. ⚠️ It used to sit outside that column, anchored to its line and stepping out a lane whenever it landed on a length box: on a drill whose shoulder EQUALS its MIN OOH (ordinary) that cost a whole 94px lane and pushed the drawing past the width its container has. The overhang is deliberately **asymmetric**: it reaches left toward the column, and the right side stops short of `GAP`, where the diameter leaders start. Nothing is calculated from the holder — some tool types could be, and deliberately are not; the number is the shop's.
...
⚠️ **EVERY LENGTH SHARES ONE COLUMN, AND THE LEADER DOG-LEGS TO REACH ITS BOX.** Values that land at the same height — a drill whose shoulder IS its MIN OOH, which is ordinary — used to fan sideways into extra lanes. That is not what an ordinate dimension does, and at 94px a lane it took the drawing past the width its container has: three crowded values pushed the Cutter and Shaft Segments panels below the drawing instead of beside it. A print keeps the column and jogs the leader, so the box moves **up or down** until it clears its neighbour and the leader runs out from the part at the **true** height, doglegs across, and enters the box at its displaced one. Two rules inside that: the top box is clamped into the canvas and only what it then collides with is pushed (shifting the whole run instead displaced every box on every tool, because OAL's true height IS the top edge and its centred box always clamps); and a leader stays **straight** while the true height still falls inside the box, so that same OAL clamp never draws a jog. Locked by `toolProfileUi.test.jsx`, which asserts one column, no vertical overlap, that some leaders dogleg and some do not, and — the symptom that started it — that a crowded tool still fits beside its panels.
...
⚠️ **THE VALUE BOXES ARE THE KEY — there is no swatch legend.** Each box borders in the colour that OUTLINES its region in the drawing (`REGION_OF`), so the box and the shape it names match exactly; a legend underneath is worse, because reading it means looking away from the thing it explains. Two rules that keep it honest: a box naming **no single region** (OAL spans the whole tool, REACH spans the flutes plus the neck) stays **neutral** rather than claiming a part it does not measure, and **shank and segment stay close to each other** — they are close in the drawing too, and inventing a distinction here would assert one the drawing does not have. The border is **1.5px**: at a hairline the colour did not carry on a box that small and every one read as the default grey, which is the legend problem again in a new place.
...
- ⚠️ **`smartDiam` TAKES THE RECORD'S UNIT, and a millimetres tool never touches the inch reference charts.** `smartDiam(value, inputWasMm, isDrillType, unit)` — `value` is in the record's OWN unit, like every length in this app (there is no hidden inches canonical), so a 6mm tool arrives as `6`. Treating that as inches is not a rounding error, it is a **different tool**: `isLikelyMetric(6)` computed 6 × 25.4 and named the library's 6mm ball mill **`152.4mm (6) BALL 4FL 12LOC`**. `FRACS`, `NUM_DRILLS` and `LETTER_DRILLS` are inch charts, so a metric record skips all three and simply says its own size (`6mm`) — **no parenthetical**, because the parenthetical exists to give the actual stored value beside a nominal label, and here the label IS the stored value.
  - **`inputWasMm` is a different question and applies only to an INCH record** — "stored in inches, but the shop calls it by its metric size" (an 8mm tool stored as `.3150"` → `8mm (.315)`). It is meaningless on a record already in mm, so the metric branch wins first; neither is ever inferred from the other.
  - **`snapTol(unit)` is RETIRED — there is one tolerance, `SNAP_TOL_IN = 0.0003"`, and it is always inches** because what it compares against is always inches. Scaling it for a metric tool read as unit-awareness and was the opposite: it kept a millimetre value pointed at those inch numbers and merely widened the window, so a **1mm** tool matched the `1` fraction and a **2.5mm** tool matched `2-1/2`. `toFrac(d, tol)` / `isLikelyMetric(inches, tol)` still take a tolerance defaulting to `SNAP_TOL_IN`; nothing passes anything else.
  - Genuine sizes store within ~0.0001" of nominal, so 0.0003" keeps them while rejecting near-misses: without a gate, snapping to the nearest 1/64 called `.0571"` (a 1.45mm tool, 0.0054" from 1/16") "1/16", and that false fraction match also suppressed `isLikelyMetric` (which bails if `toFrac` returns anything), silently dropping the metric label.
  - ⚠️ **Everything else in a generated name needs no conversion** — `loc`, corner radius and reach are printed as the record's own number in its own unit, which is correct in either. Only the diameter had a conversion in it, which is why only the diameter was wrong.
  - Locked by `toolNaming.test.js` (`buildDesc on a MILLIMETRES tool`), including that an inch tool is byte-for-byte unchanged.
...
in exactly two places — `presetMatchesAssembly` and `shouldRetireAsmNumber` /
`updateAssembly`. It never picks a holder.
...
- **✅ A holder DUPLICATED inside Fusion — resolved end to end.** Fusion's duplicate copies everything user-visible **including `product-id`, where the app's `holder_ref` lives**, so the copy arrives wearing the ORIGINAL's ID; edit it (the usual reason to duplicate one) and its shape differs too. Confirmed against the shop's real library, which contains exactly one: `HLD-604953` stamped on both a `…EX 2.35OOH` and a `…EX 2.8OOH` holder. **Detection already worked** (`isFusionSideCopy` → the push flags `fusion-copy`); what was missing was any way to act on it, so the holder could never enter the app. Three rules close it, and each one a looser version gets destructively wrong:
  - ⚠️ **AN APP-MINTED REF IS NEVER MIGRATED** (`triageProductId`, `holderRecord.js`). `fusionHolderToRecord` migrates whatever was in `product-id` into `legacy_ids` — right for a vendor SKU or prose, **catastrophic** for a ref a LIVE record still owns. **This was a destructive latent bug**: importing the copy filed the original's ref as the copy's legacy id, `deletionReason` read both entries as merged away, and the push planned to **DELETE the original holder AND the copy from Fusion and re-add them with fresh guids** — severing the baked holder link on every tool using them. Measured on the real library, that was a 2-delete / 2-create plan. Nothing but this app writes the `HLD-` shape, so it is never a SKU: it is now treated as nothing to migrate.
  - ⚠️ **A REF A LIVE RECORD STILL OWNS WAS NEVER RETIRED** (`deletionReason`). The `HLD-` shape separates an app-minted ref from a vendor SKU but **cannot** separate "a ref I retired in a merge" from "a ref I copied off a Fusion entry at import". A merge ARCHIVES the loser, so a genuinely retired ref is nobody's live `holder_ref` — the guard costs the merge case nothing and is the second, independent line of defence against the delete above.
  - ⚠️ **`conflictResolvableToShape` — THE ONE CASE WHERE THE APP MAY OVERWRITE A FUSION HOLDER'S ID.** Once the copy has its own record the entry reads **`conflict`** (our ref says one holder, the shape says another), which is normally unanswerable and stays flagged — a wrong answer renames a real holder in Fusion. It becomes **provable** in exactly one shape: the shape matches **exactly one** record, AND the record the ref names is matched on **both** signals by a **different** entry, so that holder is accounted for and this ref is provably borrowed. The push then re-stamps `product-id` as a `reclaim` update. `refClaimedElsewhere` is the shared proof behind this and `isFusionSideCopy`.
  **The flow, measured on the real library:** today 1 flagged and stuck → **Import** adopts it as its own holder (clean record, no borrowed ref) → the auto-push **reclaims** its Fusion id (`HLD-604953` → its own) → **0 flagged, 0 creates, 0 deletes**, and a second push is a no-op. The import reports copies separately (`result.copies`) rather than slipping them in — the user is adding a holder they did not know about and the push is rewriting an id in Fusion; both are news. The `duplicate-entry` path still covers "duplicated but NOT edited" (both entries `exact` → first written, rest flagged), and `adopt` still covers "product-id cleared and not edited". Locked by `holderIdentity.test.js` (incl. the delete guard, the genuine-merge case that must still delete, reclaim, and idempotence) + `holderRecord.test.js`.
  **Still unobserved, and still must not be relied on:** how Fusion handles the **guid** on duplicate. Nothing in this path reads it, which is why the resolution is safe without knowing.
