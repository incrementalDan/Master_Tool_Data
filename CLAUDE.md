...
**Strategy label (new-format milling only)** — `composePresetName` appends a short **strategy token** after the operation word, kept deliberately small so the name never balloons (`presetStrategyLabel`, `src/schema/camStrategies.js`): the selection is exactly a quick **group** → its short `nameToken` (`Adaptive`/`Facing`/`3D`/`Engrave`); **one** strategy → that strategy's name (`Bore`, `2D Contour`); exactly the **two pinned picks** (2D Contour + Bore) → both (`2D Contour + Bore`); **anything else** (2+ arbitrary strategies) → nothing (the user names it by hand). The suffix is display-only — `parsePresetName` still parses the operation from the op word before it (strategy tokens aren't op aliases), and holder/OOH live in the head, so matching is unaffected. `EditCard.composeName` passes the just-changed selection into the label so it tracks strategy edits live. Holder short names (strip `NBT`, drop the `C` after `SK<n>`, + override map) come from `src/utils/holderNaming.js`. `operation_type` is stored on the in-memory preset and cached in metadata (`preset_meta`), but is **never written into the Fusion JSON** (Fusion validates strictly) — it lives in the name. On import, operation_type is parsed from the name; the name wins on conflict.
