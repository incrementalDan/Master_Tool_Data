...
⚠️ **Two export columns are FILLED IN rather than left blank, so re-importing our own export stamps a value the app never held**: `corner_radius` `null` → `0` (`toolToExtractor`'s `?? '0'`) and `shank_diameter` `null` → the **cut diameter** (`f.shankDia || f.diameter`, which is wrong for a reduced-shank tool). Long-standing and **open** — pinned in `proShopRealExport.test.js`, which asserts they may only ever fill a missing value, never rewrite a real one.
