...
- **Presets are shown but not specially edited.** `PresetPanel`'s milling-oriented editor will still render for a probe's preset if opened (mostly-empty sliders — cosmetically odd, not dangerous), but the data-layer fix above means even hitting Save there can no longer inject the fictitious milling shape.
...
- **One-time normalize note**: running **Normalize** (`normalizeLibrary`) on a probe tool with no recognizable operation-type in its preset name ("Default preset" doesn't match any Rough/Finish/SM Bore alias) leaves `operation_type` null and the preset name untouched — the rename-to-convention step only fires when an operation type is actually resolved. If `NormalizeModal` prompts for one anyway, leaving it unassigned/skipped is the right call; forcing "Rough"/"Finish" would compose a nonsensical name for a tool that was never priced by material/operation to begin with.
