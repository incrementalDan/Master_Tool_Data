...
| Program # | `.program-num-badge` | Slightly rounded rect (r=5px) | Amber mono on dark plate (`--bg`) — CNC-screen look. Shown in its primary `O####` form via `formatProgramNumber`. Used on `/programs` (grouped rows, table, add-modal session list) |
