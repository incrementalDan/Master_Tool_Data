...
- **The parenthetical decimal is ALWAYS the tool's actual diameter, never the chart's rounded value** — a real `.0938"` drill reads `#42 (.0938)`, not `#42 (.0935)`. Never prioritize a rounded reference number over the more exact measurement.
- **`toFrac` is tolerance-gated (`FRAC_TOL = 0.001"`)** — it snaps to the nearest 1/64 **only when the value is genuinely within 0.001" of it**. Without this it snapped every value to its nearest 64th with no distance check, calling `.0571"` (a 1.45mm tool, 0.0054" from 1/16") "1/16" — and that false fraction match also suppressed `isLikelyMetric` (which bails if `toFrac` returns anything), silently dropping the metric label. Real inch fractions store within ~0.0001" of nominal, so the tight gate keeps them while letting near-fraction metric sizes (1.45mm, 6mm, 8mm) resolve to metric/decimal.
