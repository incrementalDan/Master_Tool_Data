...
**Material inference from the name**: the shop's presets encode the material **only in the name** (`"AL FIN"`, `"SS316 SM HOLE FIN"`) — Fusion's preset `material.query` is blank. `matchMaterial(str)` (`src/utils/presetNaming.js`) maps any string/code to a canonical `MATERIAL_CODES` value (`AL`/`SS`/`STEEL`/`MILD`/`BRONZE`/`BRASS`/`TI`/`CI`/`PLASTIC`) or `null` — keyword substrings (`STAINLESS`→SS, `NYLON`→PLASTIC, `LOW CARBON`→MILD…) then token codes (`SS316`/`AL-150` → SS/AL). `mergeFusionAndMetadata` sets `material.query`/`category` from `matchMaterial(p.name)` **only when Fusion left the query blank** (never overwrites a real value), so the material survives the normalize rename. `materialLabel(query)` returns the display name (used by `PresetPanel`; replaced its weaker local copy). **`"BZN"` is intentionally NOT mapped** — ambiguous (brass vs a bronze alloy); confirm before adding. Grade detail (e.g. the `316` in `SS316`) collapses to the broad code — the model has no separate grade field.
...
Presets are tinted by their material's **ISO group color**. `materialIsoGroup(query)` (`src/utils/presetNaming.js`) maps a preset's material (`matchMaterial` → canonical code) to an ISO group via `MATERIAL_CODE_TO_ISO_GROUP` (`AL`/`BRONZE`/`BRASS`→N, `SS`→M, `STEEL`/`MILD`→P, `CI`→K, `TI`→S; plastics → null/no color; hardened `H` isn't produced by `matchMaterial`, so it's never auto-assigned). `isoGroupColor(query, groups)` resolves the color from a `materials.groups` array — the single source used everywhere. `PresetPanel.jsx` (`groupColorOf`) applies it as a left-border accent on each preset card (collapsed + edit) and on the material label / group-divider dot. Preset **chips** elsewhere (`AssemblyCard` linked presets, Sync Job `DiffStep`/`CommitStep` new-preset rows) get a small leading material dot via the shared `<PresetDot query groups>` component (`src/components/PresetDot.jsx`) — the `.preset-tag` chip itself stays emerald (the per-data-type token is unchanged; the dot is additive). Unknown materials or groups with no color → no accent/dot (silent fallback).
