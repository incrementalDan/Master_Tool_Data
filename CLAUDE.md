...
The app has **three parallel, configurable identification systems**, designed to follow the **same** architectural principles so a user (and a developer) who understands one immediately understands the others. The canonical intent doc is **`THREE SYSTEM CONTEXT PROMPT.md`** (repo root) — read it before touching any ID, location, or assembly numbering feature.
...
⚠️ **The word "job" here is NOT the shop's "job".** In this section a *job* is a lightweight (program #, part #) **where-used link** that a preset or tool points at. The shop's domain "job" is a **combination of operations within a part**, which does not exist as a record yet — see **The Parts module** under TODO / Future Work. The collision has to be resolved before that tier is built; this array is already referenced from tool metadata, which is the expensive place to be wrong.
...
- **Capture — manual (same picker everywhere)**: adding a job link on a preset or a tool uses the **same `JobProgramPicker`** as Sync Job (`src/components/JobProgramPicker.jsx` — shared, not merge-specific; `SelectedProgramChip` is its held-selection summary, used only by CommitStep). Each preset card (collapsed AND edit mode) ends with a **`PresetJobsBlock`** dropdown row — `Briefcase` icon + `Jobs (N)` count always visible **without opening**, expanding to job labels; edit mode adds remove `×` and, via the picker, links a program (`onAddProgram(selection)` → `findOrCreateJob(program_number, part_number, by, program_id)` → append to `draft.job_ids`). Tool level: the **`JobsSection.jsx`** panel ("Jobs / Where Used", ToolDetail right column) aggregates preset-proven links (shown with `.preset-tag` chips naming the preset, managed on the preset) + tool-level links (edit mode shows the picker; a pick resolves the same way and appends to `tool.job_ids` via `saveTool`), deduped by job id via `collectToolJobs`. Count shown in the panel header. There is **no** free-text program/part entry anywhere anymore — every link is to a real `programs[]` record (or one created on the spot via the picker's "Add new program").
...
- **Demo**: `src/demo/demo_jobs.json` seeds two jobs, linked to FTL-000001's presets and FTL-000002 (tool-level) so the dropdown/panel render with data in `?demo=true` — plus two parts + three programs for the Program Number Manager.

### Program Number Manager (`/programs`)

The page that replaces the manually-managed Google Sheet assigning unique CNC program numbers per part / operation / machine. UX ported from the prototype `docs/ProgramNumberManager.tsx` (main branch — behavior reference, restyled to the ToolDex design system). Component: `src/components/ProgramsPage.jsx`; pure helpers + rules: `src/utils/programs.js` (tested in `programs.test.js`); top-bar tab **Programs** (`Hash` icon).

- **Data model** (in `jobs.json` v2, all stable UUIDs — SQLite-ready):
  - `parts[]`: `{ id, part_number, customer, rev, material_id, material_custom, created_at, created_by }`
  - `programs[]`: `{ id, program_number (int, permanent), part_id, operation, description, machine_id, machine_label, is_fixture, internal_external, fixturing, material_id, material_custom, pallet, created_at, created_by }`
- **Program numbers are global + permanent**: one shop-wide counter, next = `max(existing) + 1` (computed, never stored — can't drift; gaps fine). Reserved the instant an operation is added; `updateProgram` strips `program_number` from any patch. When `programs[]` is empty the Add modal exposes a one-time **"First program number"** seed input (migrating off the Sheet mid-count).
- **Primary display/reference form is `O####`** (e.g. `O1128`, `O2352`) — the same Fanuc-style prefix the machine control and G-code file names require, so the app references program numbers the way the shop already does everywhere else. `program_number` is **always stored as a plain integer** (sort / max+1 / dedupe / CSV all operate on the int) — the `O` is display-only, applied by `formatProgramNumber(n)` (`src/utils/programs.js`), the single formatter used by `ProgramNumBadge`, the "Next #" display, "Reserve program number …", import summary text, and `jobLabel()` (`src/utils/jobs.js`, for the older `jobs[]` quick-link registry). It's **idempotent** — a legacy `jobs[]` entry that already has a hand-typed `"O1042"` isn't double-prefixed. `searchPrograms()` and the CSV importer (`buildProgramsImport`) both strip a leading `o`/`O` before parsing, so typing/pasting the number in its primary `O####` form still resolves/imports correctly.
- **Operation display form is `OP##`** (e.g. `OP50`, `OP60`, `OP50R`, `OP160RB`) — `formatOperation(op)` (`src/utils/programs.js`) prefixes a bare number+optional-trailing-letters token (`50`, `50R`, `51M`, `160RB`) with `OP`, is idempotent on an already-prefixed value (any case/spacing), and leaves anything that isn't that token shape (e.g. a fixture step like `"Soft Jaw"`, or `"PRE OP"`) untouched — there's nothing to prefix. Applied at every display site: the grouped-view operation header (`.pn-op-label`), the table's Operation column, the `JobProgramPicker` result rows + `SelectedProgramChip`, and the Add-modal's "Reserved this session" list. The stored `operation` field itself is untouched free text (editable inputs show the raw value, not the formatted one).
- **Material = a SPECIFIC ALLOY**: `material_id` references the Materials library's `materials[]` (alloy) tier — not a CAM preset or ISO group. `material_custom` is the free-text escape for one-offs (`material_id` null); resolve labels via `alloyLabel` (dangling id → `"(unknown alloy)"`). Add new alloys on the Materials page — no material management UI here.
- **Material lives on the Part; fixtures carry their own** — implemented by DERIVATION, not the prototype's cascade: **non-fixture programs store NO material** (`newProgram`/`programFieldsOf` force `material_id: null`); their material is read from the part at render (`programMaterial`). Editing a part's material therefore "cascades" everywhere by construction with zero copies to drift. Only `is_fixture` programs store `material_id`/`material_custom` (fixture material — soft jaws etc.).
- **"Fixture OP?" toggle** (exact label): ON forces `internal_external: 'Internal'` + swaps in the Fixture material field; OFF returns to part-inherited material + pickable Internal/External.
- **Machines** come from `shop_settings.machines[]` (`machineOptions` — stores `machine_id` + a `machine_label` cache so rows survive machine deletion); falls back to the hardcoded `['Brother M300X3', 'Brother R650']` when none are configured. **Pallet** (1/2) only shows/persists when the machine label matches `/r650/i` (`isPalletMachine`).
- **Fixturing**: ToolDex has no first-class fixture list yet (the real one lives in ProShop) — `FIXTURING_OPTIONS` constant + Custom… free text stands in until that integration.
...
- **Views**: Grouped (Part → Operation → machine rows, **expanded by default** — the program numbers are the main thing the page exists to show; a `collapsed` Set tracks the parts the user explicitly closes) and Table (sortable/filterable, same inline edit). Everything editable after creation EXCEPT `program_number`. Edits gated on `googleAuthenticated || demoMode` (read-only note otherwise). All writes go through `saveJobs` (the shared-file optimistic + debounced layer — that layer IS the data-access seam for the future SQLite swap).
- **Deleting a program**: a `Trash2` icon next to the edit pencil (both views) requires an inline "Delete O####? / Delete / Cancel" confirm before `onDeleteProgram` fires (mirrors the Settings machine-list delete-confirm pattern — no browser `window.confirm`, no modal). Since `nextProgramNumber` is always computed live as `max(existing) + 1` and never stored, deleting a program is automatically safe with no extra bookkeeping: removing a non-max entry leaves the max — and "Next #" — untouched, while removing the current highest entry naturally recomputes "Next #" down to reclaim it. Locked by a `programs.test.js` case covering both outcomes. No cross-reference cleanup — a `jobs[]` link's `program_id` can be left dangling by a delete, same tolerance as any other dangling-id reference in the app (`alloyLabel`, `collectToolJobs`).
- **Deleting a part**: a `Trash2` icon next to the part's edit pencil (grouped view header) with the same inline confirm pattern; `onDeletePart` removes the part **and all of its programs** in one `saveJobs` write. The freed numbers behave exactly like `deleteProgram`'s (next # recomputes live); dangling `jobs[]` references are tolerated the same way.
- **Data tokens**: `.program-num-badge` (dark plate + amber mono — the Program # token, add to the token table below; sized 40% larger than the base data-field scale — 15px / 2px 10px padding / 7px radius — at the user's request), `.customer-badge`, `.pn-type-pill` (fixture amber / external green / internal neutral).
- **Icon coloring**: the page's functional icons (view-tab Grid/Table, part-expand chevron, table-search glass, sort arrows) are tinted `var(--blue)` rather than inheriting the surrounding label/text color — same accent already used for `.panel-header-icon`/`.detail-header-icon` elsewhere, so an icon reads as an icon rather than blending into the text next to it. Scoped via `.pn-view-tab > svg`, `.pn-part-header > svg`, `.pn-search svg`, `.pn-table th svg` (direct-child selectors where a nested `.icon-btn` needed to keep its own hover/active coloring untouched).
- **CSV import (one-time)**: **Settings → Import Program List** loads the shop's existing list. Header (order-independent, aliases tolerated): `Program #, Machine, Fixturing, Internal or external, internal Part #, Rev, Customer, Description, OP #, Fixture Y/N`. Parser + build in `src/utils/programsImport.js` (`parseProgramsCsv`, `buildProgramsImport`; tested in `programsImport.test.js`); modal `ProgramsImportModal.jsx` (file → preview counts/duplicates/errors → commit via `saveJobs`). Rules: each row = one program; rows sharing **(part_number, rev)** group into one part (matched case-insensitively against existing parts → reused by id, never duplicated); `Fixture Y/N` → `is_fixture` (Y/Yes/T/1/…), which forces `internal_external: 'Internal'`; machine string resolved against `shop_settings.machines` (exact→contains, else raw label + null id); a leading `O` on a Program # is stripped (the primary `O####` reference form); **program_number is the dedupe key** — a number already in the registry (or earlier in the file) is skipped; a **blank** Program # is auto-assigned the next available (max+1, running); a non-integer Program # is an error (row skipped, reported). Material/pallet aren't in the CSV (left unset — add material on the part later). Gated on `googleAuthenticated || demoMode`. **CSV parsing is a full-file quote-aware tokenizer (`parseCsvRows`)** — it honors quoted fields that contain commas, escaped quotes, AND embedded line breaks. Real exports from this shop wrap the `"Program #"` header cell with a leading newline (`"\nProgram #"`); a naive line-split-first parser shatters that cell and drops every column (surfacing as a bogus "no Part # column" error), so the tokenizer only treats `\n` as a row break when not inside quotes.
...
- **The Parts module — Part → Job → OP → Program (direction, not yet built).** The Program Number Manager is becoming a **parts module**: the part is the entity the work hangs off, and the number registry is one view of it rather than the container. The shop's model, in their words:

  | Tier | What it is | Today |
  |---|---|---|
  | **Part** | the thing being made | `parts[]` — real record ✅ |
  | **Job** | a combination of operations. **A part can have more than one** | ❌ does not exist |
  | **OP** | a sequential step, *usually* with its own program | a free-text `operation` string on the program |
  | **Program** | the posted NC program + its number | `programs[]` — real record ✅ |

  So today's model is that hierarchy **flattened**: a program carries `part_id` + an `operation` string, and Job doesn't exist at all. That flattening is not wrong — it is the same data with two tiers implicit — and everything built so far (the part page, tool lists, labels, sequence detail) hangs off `part_id`, which survives the change. Adding a nullable `job_id` to a program later is a column plus a grouping in the UI, not a rewrite.

  ⚠️ **The ONE thing to settle before more is built on the word "job": `jobs[]` in `jobs.json` already means something else.** It is the lightweight (program #, part #) **where-used link registry** that presets and tools point at (`preset_meta.job_ids`, tool `job_ids`, `collectToolJobs`, `JobsSection` "Jobs / Where Used", `findOrCreateJob`). If the new Part→Job tier is also called "job", the same file has two unrelated "jobs" — and the older one is already referenced from tool metadata, which is the expensive place to be wrong. Do not build the new tier until this is resolved. Three ways out, in order of preference:
    1. **Retire the old `jobs[]` entirely.** Its real purpose is "which program was this preset proven on", and `jobs[].program_id` already joins to a `programs[]` record — so presets/tools could reference **`program_id` directly** and the array disappears. Cleanest end state; needs a migration of `preset_meta.job_ids` → `program_ids` and is **backwards-incompatible** with metadata already written.
    2. **Rename the old one** (`where_used[]` / `usage_links[]`), freeing "job" for the domain concept. Same migration cost, less conceptual cleanup.
    3. Name the new tier something else. **Rejected unless the shop's own vocabulary changes** — fighting the words the floor actually uses is how a module stops matching the work.

  **Other things to decide with real jobs in front of you, not in the abstract:**
    - **What distinguishes two jobs on one part?** A different fixturing/workholding approach, a different machine, a process revision, a run/quantity? The answer determines whether Job carries its own fields or is just an ordered grouping of OPs.
    - **Does OP become a record?** "*Usually* will have its own program" implies steps that have none (inspection, deburr, an outside process), plus a real ordering and possibly two programs under one OP. Today `operation` is a string sorted numerically, which handles the common case and nothing else. Making it an entity is a migration of every program row.
    - **Part vs revision.** `parts[]` treats (part_number, rev) as a distinct part — a new rev is a new record with its own programs. A parts module probably wants Part → Revisions instead, which changes what the part page is a page *of*.
    - **Does `/programs` stay the top-level tab, or does a Parts tab become the entry point** with the number registry inside it? The part page already outgrew being a detail view of a program list.

  **Recommendation: keep building on `part_id` and the free-text `operation` for now.** Both survive the change, and the shape of Job is much better decided by looking at three or four real multi-job parts than by guessing. The naming collision above is the exception — settle that first, because every day it stays unsettled is another reference written against the ambiguous word. Small cleanup that comes with it: the label/tool-list helpers are named `jobLabelRows` / "job tool list" from the earlier framing and should follow whatever the vocabulary lands on.
