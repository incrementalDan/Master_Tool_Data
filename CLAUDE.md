...
| `product_link` | `product-link` (+ `expressions.tool_productLink`) | — | The **ProShop tool page URL**, composed from `tool_id` (`A-25` → `…/procnc/tools/A/A-25$`) by `proShopLinkForWrite` (`src/utils/proShopUrl.js`) at every `writeLogicalTool`. ⚠️ **Fills a blank and corrects a link left on the OLD number after a renumber — never overwrites anything else**: a link already pointing at this tool (11 real ones carry a pasted browser session tail, 1 has no `$` — same page, not wrong), a ProShop **RTA** link, or the **manufacturer product page** a scanned spec sheet puts in this same field. Skipped for a **combined insert id** (its ProShop page is `/procnc/rtas/{year}/{n}$`, which can't be composed) and in any non-`proshop` ID mode (the Tool ID isn't a ProShop number there). Locked by `proShopUrl.test.js`, incl. "would rewrite NOTHING in the existing library" |
