...
⚠️ **A DERIVED DIMENSION IS READ-ONLY IN THE UI** — Reach and Undercut Ø render as read-outs (dashed border, "from the shaft segments") wherever the segments answer them, exactly like the segment table. Rendering them as inputs invites a value the next load silently re-derives away, which is the failure this whole rule exists to remove. They become editable only where Fusion drew no shaft.
