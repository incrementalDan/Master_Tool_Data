...
- ⚠️ **`stock-materials` is corrected, never invented or clobbered.** It is the link Fusion actually reads (matched by name). Absent/empty → **left absent** (`normalizePreset`'s standing rule: a name Fusion can't resolve is worse than none). Equal to the OLD query → corrected alongside it. **Anything else** — a different single name (`["SS Harder"]` sat next to query `"SS Austenitic 316"` on 6 real presets) or a multi-value assignment — is somebody's Fusion-side choice: **kept, and reported** in the dialog's `kept[]` so the push never quietly does less than it claims. The name is still corrected in either case.
