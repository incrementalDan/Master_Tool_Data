...
- **Insert-tool ProShop import → don't create a Fusion-only placeholder for a component number (fix later, tie-in to the "zero Fusion instances" item above).** When the insert-tool ProShop wiring lands (Step 2b — route a ProShop row whose `Tool #` matches one side of a combined `holder/insert` product-id to a **component** record instead of a tool), a component row must NOT fall through to `psRowToTool` and mint a standalone placeholder tool in the Fusion library (a holder-only / insert-only entry Fusion should never have — the whole reason components are a separate metadata-only store). Short-term 2b handles this by intercepting component-number rows before the placeholder path; the clean long-term fix is the same "logical record with zero Fusion instances" support described in the bullet above (a component is exactly that — a real record with no Fusion entry). Until then, keep the intercept tight so no component row leaks into `saveFullLibrary`.
