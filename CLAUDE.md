...
A **reach** tool keeps its cutting diameter (or a hair under it) for some distance **above** the flutes, so it can drop into a deep pocket without the shank rubbing. An **undercut** is that neck ground a little *under* the cutting diameter. Both are **metadata-only** (`reach`, `has_undercut`, `undercut_diameter` — `NO_BORING_TURN`, i.e. every milling and hole-making type), and both are **derived at load** from the shaft segments Fusion already carries. Pure helpers: `src/utils/toolReach.js`.
...
⚠️ **AN UNDERCUT IS A NECK GROUND *A LITTLE* UNDER THE CUT — a bare `neck < diameter` test calls every slitting saw one.** A saw arbor, a lollipop stem, a dovetail shank and a face-mill body are all narrower than the cut; none is an undercut. Measured across the real 303-tool library, every genuine undercut runs **96.0–97.4%** of the cutting diameter (.038 of .039, .0605 of .0625, .09 of .09375, .0192 of .02) and the nearest thing that is *not* one is a 2" face mill's 1.75" body at **87.5%**, dropping to 61.3% (lollipop) and 40% (saw). **`UNDERCUT_MIN_RATIO = 0.92`** sits in the middle of that gap. Undercut is an inherently *small* relief; a neck 12% under is a different-diameter part of the tool, not a relieved one.

**`backfillReach(tools)`** is the load-time seed (all five `loadTools` build sites, incl. demo and local mode), mirroring `backfillAsmNumbers` / `backfillMaterialPresetIds`: in memory, persisted lazily on the tool's next save. It **only ever fills a blank** — `null`/`undefined` is "nobody has said", and anything else (including a `false` the user ticked off a mis-flagged saw) is an answer and is left alone, so the pill can always be turned off and stay off. It returns the **same array and the same tool references** when nothing is missing, so callers can use identity to tell there is nothing to persist. Idempotent. Measured on the real library: **22 tools seeded with a reach, 7 flagged undercut**, second run a no-op.
...
- **Reach & undercut, Phase 2 — the segments themselves.** Phase 1 shipped the metadata fields + the load-time derivation (see **Reach & undercut**); `shaft.segments[]` already round-trips untouched, so everything below is additive. Open items, roughly in order of value:
  - **A segment editor.** Today segments can only be drawn in Fusion. The holder module's `SegmentTable` (`HolderDetail`) is the model — including its two hard-won UI rules: the `+` sits on the row **border** (absolutely positioned, so toggling edit mode doesn't resize the table) and the delete `×` is **hidden, not removed** (keeps the column width). ⚠️ Note the ordering trap: `insertSegmentAt` exists because the display is the stored array **reversed**, and getting it wrong silently puts the segment on the opposite end of the tool.
  - **Writing segments back to Fusion.** A tool-scoped write already rebuilds the entry, so a `shaft` the app edited would need the same "write native + expression together" care every other Fusion field gets — check whether Fusion carries a paired expression for the shaft profile before trusting a bare write.
  - **The tools that recorded reach in `shoulder_length` with no usable neck segment** — `1mm (.039") Ball 3FL EM .059LOC 7x Reach` (SL .2805, LCF .059) and `.098Ø #6 Threadmill .330 reach` (SL .33, LCF .05). The seed can't reach them (nothing in their segments says so) and `normalizeLibrary` sets `shoulder_length = min_ooh`, which will overwrite it. A handful of tools; either type the reach in by hand first, or add a one-off rescue before the next normalize.
  - **Reach on a tool whose "neck" IS its body.** The rule is literal, so a 2" face mill's 1.75" body reads as `1.16` reach and a saw arbor as `3.66` — true by the definition, but the description suggestion (`2 (2) FACE MILL 1.16 REACH`) is noise on those. Decide whether the *description* should skip types that don't print LOC, or whether the shop wants it there. The field value is correct either way, and a suggestion is never auto-applied.
