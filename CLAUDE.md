...
⚠️ **NOTHING INFERS WHETHER A TOOL HAS AN UNDERCUT — the pill is MANUAL.** An earlier pass auto-flagged it by ratio: real undercuts clustered at 96–97% of the cutting diameter and saw arbors / lollipop stems at 61% and below, so a 92% threshold separated them. That was rejected, correctly. It is a rule reverse-engineered from a gap that happens to exist in today's 303 tools — wrong the first time a tool lands in between — and it is a claim about tool geometry the app does not model. **Deriving a rule from one dataset and applying it as fact is exactly how the app starts disagreeing with Fusion about tools Fusion has right.** Do not re-add a threshold.

What the app *may* do is read a number back: once a person answers that there IS an undercut, the smallest neck diameter in the shaft segments is by definition its diameter. `undercutDiameterHint(tool)` supplies it as an **editable prefill** when the pill is switched on, and never overwrites a typed value. Reading is a fact; deciding was the answer just given.

**`backfillReach(tools)`** is the load-time seed (all five `loadTools` build sites, incl. demo and local mode), mirroring `backfillAsmNumbers` / `backfillMaterialPresetIds`: in memory, persisted lazily on the tool's next save. **Reach only** — it never touches the undercut fields. It **only ever fills a blank** (`null`/`undefined` is "nobody has said"; anything else is an answer) and returns the **same array and the same tool references** when nothing is missing, so callers can use identity to tell there is nothing to persist. Idempotent. Measured on the real library: **20 tools seeded**, 0 face mills, 0 undercuts set, second run a no-op.
