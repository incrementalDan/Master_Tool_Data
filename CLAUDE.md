...
**Linking the existing cutting tools to the controlled holders** (`backfillHolderIds`, `holderResolve.js` — load-time, in memory, persisted on each tool's next save). A tool carries a baked copy of its holder, so the link is re-derived from what it carries, in this order: the assembly's **`holder_id`** if it still resolves → the baked **guid** → the baked **SEGMENTS**, matched with the same strict rule used at the Fusion boundary and applied only when **exactly one** record has that shape. The segment step is not optional: measured against the shop's own data, **the guid links 45% of tools and the shape links 93%** (0 ambiguous — all 20 records have distinct shapes). A tool matching neither is left alone for **Link tools to holders** (below).
