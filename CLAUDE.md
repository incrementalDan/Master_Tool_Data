...
The write is **metadata-only** (`holder_id` is a metadata field) — a batch `toolStore.upsertMany`, never a Fusion round-trip per tool. Corrected geometry reaches Fusion on each tool's next save, or via Re-stamp.
