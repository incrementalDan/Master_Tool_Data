...
- 🔨 **Phase B (in progress)** — where behavior deliberately extends. Decomposed:
  ...
  - ⏳ **B4b (next)** — onboarding `integrations.fusion.enabled` toggle + the fully-Fusion-disabled load mode (relax the "no library linked" hard error; skip the Fusion download loop when disabled). Touches the `App.jsx` gates.
