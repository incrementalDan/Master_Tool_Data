...
- 🔨 **Phase B (in progress)** — where behavior deliberately extends. Decomposed:
  ...
  - ✅ **B6 — mode-2 load: the app's own store is primary (done, 2026-07-15).** `loadTools` is now two-stage: **stage 1** builds and paints the whole library from the complete metadata records IMMEDIATELY (`isCompleteRecord` filter → `buildUnlinkedTool` + location compose + pairing/asm backfill → `LOAD_PROVISIONAL`), then **stage 2** runs the exact pre-existing Fusion build and `LOAD_SUCCESS` replaces it — Google paints, Fusion confirms. Write safety: `fusionReady` (set only by a completed full build; demo/local set it too) gates `writeLogicalTool`'s linked path, `saveFullLibrary`, and reconcile-on-open with a clear "still syncing" refusal — provisional tools have no `_instancesRaw` base, so the 3-way merges must never run against them. A **one-time complete-record backfill** after stage 2 (`recordsNeedingBackfill` → `toolStore.upsertMany`) writes the complete record for every built tool whose stored record is missing or still the pre-increment overlay shape — keyed off BUILT tools so orphan metadata stays dormant (ghost guard intact); skipped when the metadata file is missing/trashed. Fidelity locked by `completeRecord.test.js` (the "preset sync test"): Fusion build → `buildMetadataTool` → `buildUnlinkedTool` reproduces scalars, full presets (speeds/feeds + native keys + expressions + the app-only overlay), assemblies, and the O1 flat mirror. 331 tests + audit green.
  ...
  - ✅ **Write-time concurrent-edit safety (done — generalizes the preset-wipe fix).** `writeLogicalTool` now 3-way merges **presets** (`mergePresetsWithFusion`), **shared fields** (`mergeSharedFieldsWithFusion` over `DRIFT_FIELDS` — geometry/identity/material), and **per-instance OOH + holder** (`mergeInstanceFieldsWithFusion`) before writing: if Fusion changed a value since the app loaded and the app didn't, adopt Fusion's — so a save never silently wipes a Fusion edit even without a reload (the drift banner is the load-time surface; this is the write-time net). The rare both-edited-the-same-thing conflict keeps the app's version. 222 tests + audit + build green.
  - ✅ **ID-system membership (done — replaces the accidental no-Fusion skip in bulk ID ops).** Every tool is a **member of all three ID systems** (Tool ID / Machine Number / Location) by default; `id_system_exclusions` (metadata) is an explicit, per-system, reversible opt-out (`src/utils/idSystems.js`). (1) The bulk ops (`assignToolIds` / `renumberAllToolIds` / `renumberLibrary` / `normalizeLocationSystem`) now process no-Fusion tools (metadata-only) and skip only excluded ones. (2) Settings → **ID System Membership** panel lists exclusions per system with Add-back + a search to exclude. (3) An **ExclusionNotice** in each bulk op's preview reminds the user of exclusions with a one-click "Include all in this run." So a tool never falls out of a system silently, and any skip is deliberate + visible + reversible. 230 tests + audit + build green.
  ...
  - ✅ **Write-time both-edited conflict surfacing (done — closes the last silent-app-wins gap).** The write-time 3-way merges keep the app's active edit when *both* the app and Fusion changed the same thing (correct — it's the edit the user is saving), but that used to be **silent**. Now all three merges (`mergePresetsWithFusion` / `mergeSharedFieldsWithFusion` / `mergeInstanceFieldsWithFusion`) take an optional `conflicts` accumulator and record every both-edited case (`{field,appValue,fusionValue}` for scalars; `{kind:'preset',preset}` / `{kind:'ooh'|'holder',assembly}` for the rest). `writeLogicalTool` collects them and (1) raises a **warning toast** summarizing what Fusion also changed, and (2) attaches the scalar-field conflicts to the saved tool's `_drift` so the **DriftBanner** re-surfaces them — Fusion's value stays one click away (Keep-Fusion restores it). A clean save clears stale `_drift` to `[]`. So D3 now holds on the *write* path too, not just load-time drift. 234 tests + audit + build green.
...
- **Two surfaces, both non-silent:**
  - **Load-time** — `detectFusionDrift` scans every instance on open → `_drift` → the `DriftBanner` (above).
  - **Write-time** — the 3-way merges in `writeLogicalTool` net a Fusion edit made *after* the app loaded (no reload needed). Only-Fusion-changed → adopt it (no wipe). **Both changed the same thing → keep the app's active edit** (it's what the user is saving) **but surface it**: a `conflicts` accumulator flows through all three merges; `writeLogicalTool` raises a warning toast ("Kept your edits — Fusion also changed N fields/presets/assembly values") and re-attaches the scalar-field conflicts to `_drift` so the `DriftBanner` offers a one-click Keep-Fusion restore. So the both-edited case is never silently app-wins.
