...
- 🔨 **Phase B (in progress)** — where behavior deliberately extends. Decomposed:
  ...
  - ⏳ **B2** — wire `loadTools`: after the Fusion-built tools, append `buildUnlinkedTool` for each marked, unrepresented metadata record; relax the "no library linked" hard error when Fusion is disabled.
  - ⏳ **B3** — `writeLogicalTool` metadata-only branch for unlinked tools (+ `deleteTool` / bulk-op partitioning). Note the interaction with ProShop import's current placeholder-minting (`no_fusion_link` today still gets a Fusion placeholder — that flips here / in Phase C).
  - ⏳ **B4** — UI: "Not in Fusion" badge, "Create Fusion entry" (promote) / "Detach from Fusion" (demote), onboarding `integrations.fusion.enabled` toggle.
  - ⏳ **B5** — D2 authority read-branch + D3 drift surfacing.
