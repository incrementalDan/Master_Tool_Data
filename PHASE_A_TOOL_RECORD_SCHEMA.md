...
- 🔨 **Phase B (in progress)** — where behavior deliberately extends. Decomposed:
  ...
  - ⏳ **B4 (next)** — UI: "Not in Fusion" badge, "Create Fusion entry" (promote) / "Detach from Fusion" (demote), onboarding `integrations.fusion.enabled` toggle + the fully-Fusion-disabled load mode.
