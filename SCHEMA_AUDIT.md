...
- **Deferred (small):** a per-record unit picker in the tool edit UI (the model + display already support it; only the input control is missing). `computeGaugeLength` (returns inches) is now unused but kept as a utility.
